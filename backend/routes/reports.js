...
function daysUntil(expiryDate) {
    return Math.ceil((day(expiryDate) - day(new Date())) / 86400000);
}
...
function buildMedicationScope(scope) {
    return scope.orgId ? { $or: [{ orgId: scope.orgId }, { addedBy: scope.userId }] } : { addedBy: scope.userId };
}
function buildReportScope(scope) {
    return scope.orgId ? { $or: [{ orgId: scope.orgId }, { generatedBy: scope.userId }] } : { generatedBy: scope.userId };
}
...
async function buildReportData({ scope, reportType, reportSubType, filters }) {
    ...
    if (reportType === 'Expiry Reports') {
        ...
        else if (subtype === 'expiring_soon')
            rows = rows.filter(
                (m) =>
                    m.expiryDate &&
                    m.daysUntilExpiry !== null &&
                    m.daysUntilExpiry >= 0 &&
                    m.daysUntilExpiry <= Number(filters.expiryWindowDays || 30),
            );
    ...
    if (reportType === 'Stock Reports') {
        ...
        rows.sort(
            (a, b) => Number(a.currentStock || 0) - Number(b.currentStock || 0) || String(a.medicationName).localeCompare(String(b.medicationName)),
        );
        return { kind: 'table', rows };
    }
    ...
    if (reportType === 'Compliance & Safety Reports') {
        ...
        if (subtype === 'removed_expired_audit') rows = rows.filter((m) => ['Removed', 'Expired'].includes(m.derivedStatus));
            ...
            rows = rows.filter(
                (m) =>
                    ['Expired', 'Recalled', 'Removed'].includes(m.derivedStatus) ||
                    (m.expiryDate && m.daysUntilExpiry >= 0 && m.daysUntilExpiry <= 30),
            );
        rows.sort(
            (a, b) =>
                String(a.derivedStatus).localeCompare(String(b.derivedStatus)) ||
                new Date(a.expiryDate || '9999-12-31') - new Date(b.expiryDate || '9999-12-31'),
        );
        return { kind: 'table', rows };
    }
    ...
    if (reportType === 'Usage & Trends') {
        ...
        expiredRows.forEach((m) => {
            ...
            prior.itemsExpired += 1;
            prior.unitsExpired += Number(m.currentStock || 0);
            periodMap.set(period, prior);
        });
        ...
        rows.forEach((m) => {
            ...
            prior.itemCount += 1;
            prior.totalUnits += Number(m.currentStock || 0);
            statusMap.set(key, prior);
        });
        ...
        expiredRows.forEach((m) => {
            ...
            prior.itemsExpired += 1;
            categoryMap.set(key, prior);
        });

        return {
            kind: 'summary',
            expiredWasteOverTime: Array.from(periodMap.values()).sort((a, b) => a.period.localeCompare(b.period)),
            mostExpiredItems: Array.from(expiredCountMap.entries())
                .map(([medicationName, timesExpired]) => ({ medicationName, timesExpired }))
                .sort((a, b) => b.timesExpired - a.timesExpired || a.medicationName.localeCompare(b.medicationName))
                .slice(0, 15),
            inventoryStatusMix: Array.from(statusMap.values()).sort((a, b) => b.itemCount - a.itemCount || a.status.localeCompare(b.status)),
            categoryWaste: Array.from(categoryMap.values())
                .sort((a, b) => b.itemsExpired - a.itemsExpired || a.category.localeCompare(b.category))
                .slice(0, 10),
        };
    }
...
function drawHeader(doc, title, meta) {
    ...
    Object.entries(meta || {}).forEach(([k, v]) => {
        doc.font('Helvetica-Bold').fontSize(10).text(`${k}:`, 50, y, { continued: true });
    ...
    return new Promise((resolve) => {
        ...
        if (summary) {
            ...
            sections.forEach(([label, items]) => {
                ...
                y += 16;
                if (!items.length) {
                    doc.font('Helvetica').fontSize(9).fillColor('#6b7280').text('No data available', 50, y);
                    y += 18;
                    return;
                }
                items.forEach((item) => {
...
function resolveReportRecipientEmail(currentUser) {
    return String(currentUser?.email || '').trim();
}

function buildReportEmailHtml({ recipientName = 'there', reportType, format, fileUrl, generatedAt }) {
    return `
      <div style="font-family:Arial,sans-serif;max-width:620px;margin:0 auto;color:#1f2937;">
        <h2 style="color:#00808d;margin-bottom:12px;">ShelfSafe report ready</h2>
        <p>Hi ${recipientName},</p>
        <p>Your <strong>${reportType}</strong> report in <strong>${format}</strong> format has been generated successfully.</p>
        <p style="margin:16px 0;">
          <a href="${fileUrl}" style="display:inline-block;background:#00808d;color:#fff;text-decoration:none;padding:10px 16px;border-radius:8px;">Open report</a>
        </p>
        <p style="font-size:14px;color:#4b5563;">Generated at: ${generatedAt}</p>
        <p style="font-size:13px;color:#6b7280;">If the button does not work, copy this link into your browser:<br/>${fileUrl}</p>
      </div>
    `;
}

async function emailGeneratedReport({ req, scope, reportType, reportSubType = '', format, filters = {} }) {
    const recipient = resolveReportRecipientEmail(scope.currentUser);
    if (!recipient) throw new Error('No email address found for the logged-in user');
    ...
    await sendEmail({
        to: recipient,
        subject: `ShelfSafe ${reportType} (${format})`,
        html: buildReportEmailHtml({
            recipientName: scope.currentUser?.name || recipient,
            reportType,
            format,
            fileUrl: publicUrl,
            generatedAt: new Date().toLocaleString('en-US'),
        }),
        text: `Your ${reportType} report is ready: ${publicUrl}`,
    });

    return { reportDoc, publicUrl, recipient };
}
...
router.post('/email-report', verifyToken, async (req, res) => {
    try {
        const { reportType, reportSubType = '', format = 'PDF', filters = {} } = req.body || {};
        if (!reportType) return res.status(400).json({ success: false, message: 'reportType is required' });
        if (!['PDF', 'CSV'].includes(format)) return res.status(400).json({ success: false, message: 'format must be PDF or CSV' });

        const scope = await getScope(req);
        const { recipient } = await emailGeneratedReport({ req, scope, reportType, reportSubType, format, filters });
        res.status(201).json({ success: true, message: `${format} report emailed successfully to ${recipient}` });
    } catch (error) {
        res.status(500).json({ success: false, message: error.message });
    }
});

router.post('/email-pdf', verifyToken, async (req, res) => {
    try {
        const { reportType, reportSubType = '', filters = {} } = req.body || {};
        if (!reportType) return res.status(400).json({ success: false, message: 'reportType is required' });
        const scope = await getScope(req);
        const { recipient } = await emailGeneratedReport({ req, scope, reportType, reportSubType, format: 'PDF', filters });
        res.status(201).json({ success: true, message: `PDF report emailed successfully to ${recipient}` });
    } catch (error) {
        res.status(500).json({ success: false, message: error.message });
    }
});
